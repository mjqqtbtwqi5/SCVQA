...
if __name__ == '__main__':
    ...
    print(f"database: {DATABASE}, CNN module: {CNN_MODULE}, device: {DEVICE}, batch_size: {BATCH_SIZE}, num_workers: {NUM_WORKERS}, num_epochs: {NUM_EPOCHS}, seed: {SEED}")
    ...
    print(f"Number of training data: {len(train_dataset)} & testing data: {len(test_dataset)}")
    # ==================================================
    ...
    end_time = timer()
    print(f"Total training time: {datetime.timedelta(seconds=int(end_time-start_time))} (Hour:Minute:Second)")
